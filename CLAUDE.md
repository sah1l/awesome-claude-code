# awesome-claude-code — Repo Instructions

## What This Is
A showcase repository of Claude Code configuration patterns — agents, skills, commands, and hooks.
This is a reference/template repo, not an application.

## Quick Start

This repo is designed to be cloned and adapted for your projects. Here's how to use it:
...
cp -r awesome-claude-code/.claude/agents/ your-project/.claude/
cp -r awesome-claude-code/hooks/ your-project/hooks/
```

### 2. Customize
- **CLAUDE.md**: Update with your project's tech stack, architecture, and rules
- **Skills**: Adapt examples to your domain (swap Drizzle for Prisma, etc.)
- **Agents**: Configure model preferences and tools for your needs
- **Hooks**: Set up automation for your toolchain (formatter, linter, test runner)

### 3. Use
- Copy `.claude/settings.json` to your project
- Run `/config` in Claude Code to see your setup
- Use the slash commands like `/plan`, `/review-pr`, `/tdd`
...
## Writing Guidelines
- Every file should explain **WHY**, not just **WHAT** — this is a teaching repo
- Keep examples technology-agnostic unless the skill is domain-specific (e.g., migration-writer targets Drizzle)
- Use real-world scenarios, not toy examples
- Cross-reference related files (e.g., agents should reference the skills they use)

## File Conventions
- Agent files: `.claude/agents/<name>.md` — include model preference, tools, and structured output format
- Skill files: `.claude/skills/<name>/SKILL.md` — include trigger conditions and $ARGUMENTS usage where applicable
- Command files: `.claude/commands/<name>.md` — include description, workflow steps, and agent orchestration
- Hook scripts: `hooks/<name>/` — always provide both `*.sh` and `*.ps1` variants

## Quality Bar
- No placeholder content — every example should be production-ready
- All shell scripts must have correct shebang lines and be executable
- All internal markdown links must resolve correctly
